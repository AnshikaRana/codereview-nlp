...
def analyze_comments(code: str) -> dict:
    ...
    if not pairs:
        if re.search(r'\bdef \w+', code):
            ...
            suggestions.append("add docstrings or inline comments explaining why, not what")
        score = max(0, 100 - len(flags) * 15)
        return {"module": "comment_quality", "score": score, "flags": flags, "suggestions": suggestions}
    ...
    for comment, code_context in pairs:
        ...
        if len(comment.strip()) < 6:
            ...
            suggestions.append("expand the comment — explain the reason, not just what")
            continue
        ...
        if len(errors) > 1:
            ...
            suggestions.append(f"fix grammar in comment: '{short_comment}'")

        if _is_restatement(comment, code_context):
            flags.append(f"restates code: '{short_comment}'")
            suggestions.append("explain *why* this code exists, not just what it does")
