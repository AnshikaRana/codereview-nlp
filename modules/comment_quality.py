...
def _is_restatement(comment: str, code_context: str) -> bool:
    if not code_context or _model is None:
        return False
...
def analyze_comments(code: str) -> dict:
    ...
    if not pairs:
        if re.search(r'\bdef \w+', code):
            flags.add("no comments found in code with function definitions")
            suggestions.add("add docstrings or inline comments explaining why, not what")
    ...
    for comment, code_context in pairs:
        ...
        if len(comment.strip()) < 6:
            flags.add(f"too sparse: '{short_comment}'")
            suggestions.add("expand the comment — explain the reason, not just what")
            continue

        # 🔹 Grammar
        if _tool:
            grammar_issues = _tool.check(comment)
            errors = [i for i in grammar_issues if i.rule_id not in (
                "WHITESPACE_RULE", "COMMA_PARENTHESIS_WHITESPACE"
            )]

            if len(errors) > 1:
                flags.add(f"grammar issues: '{short_comment}'")
                suggestions.add("fix grammar in inline comments")

        # 🔹 Restatement
        if _is_restatement(comment, code_context):
            flags.add(f"restates code: '{short_comment}'")
            suggestions.add("explain *why* this code exists, not just what it does")
